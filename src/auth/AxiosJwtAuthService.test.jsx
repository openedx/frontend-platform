...
createCsrfTokenInterceptor.__Rewire__('httpClient', csrfTokensAxios); // eslint-disable-line no-underscore-dangle
...
const setJwtCookieTo = (jwtCookieValue) => {
  mockCookies.get.mockImplementation((cookieName) => {
    if (cookieName === process.env.ACCESS_TOKEN_COOKIE_NAME) {
...
beforeEach(() => {
  service = new AxiosJwtAuthService({
    loggingService: mockLoggingService,
    configService: mockConfigService,
  });
  service.authenticatedUser = null;
  service.httpClient = accessTokenAxios;
  client = service.authenticatedHttpClient;
  axiosMock.reset();
  ...
  csrfTokensAxiosMock
    .onGet(process.env.CSRF_TOKEN_API_PATH)
...
describe('authenticatedHttpClient usage', () => {
  describe('User is logged in', () => {
    describe('No jwt cookie exists on load', () => {
      ...
      //           .toEqual(`${global.location.origin}${process.env.CSRF_TOKEN_API_PATH}`);
      //       });
      //     });
      //   });
      // });
...
//       administrator: false,
//     };
//     axiosMock.onGet(`${process.env.LMS_BASE_URL}/api/user/v1/accounts/the_user`).reply(200, {
//       additional: 'data',
//     });
//     await service.hydrateAuthenticatedUser();
...
//       setJwtCookieTo(null);
//       return service.ensureAuthenticatedUser('/route').then((authenticatedUserAccessToken) => {
